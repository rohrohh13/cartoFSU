...
const Nav = () => {
    ...
    const { setCurrentNav, setIsDomSelected, isDomSelected } = useNavStore();
    return (
        <nav className={styles.navContainer}>
            ...
            <div>
            <button onClick={() => setIsDomSelected(false)} style={{ backgroundColor: !isDomSelected ? 'black' : 'initial', color: !isDomSelected ? 'white' : 'initial' }}>HEXAGONE</button>
            <button onClick={() => setIsDomSelected(true)} style={{ backgroundColor: isDomSelected ? 'black' : 'initial', color: isDomSelected ? 'white' : 'initial' }}>OUTRE-MER</button>                
            </div> 
        </nav>
    )
}
